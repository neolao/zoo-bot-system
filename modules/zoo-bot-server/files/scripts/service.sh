...
# ------------------------------------------------------------------------------
### BEGIN INIT INFO
# Provides:          zoo-bot-server
# Required-Start:    $remote_fs $named $syslog
# Required-Stop:     $remote_fs $named $syslog
# Default-Start:     2 3 4 5
# Default-Stop:      0 1 6
# Short-Description: DEBIAN initscript for zoo-bot-server
# Description:       DEBIAN initscript for zoo-bot-server
### END INIT INFO

PATH=/sbin:/usr/sbin:/bin:/usr/bin:/usr/local/bin
DESC="Zoo Bot Server"
CHDIR=/opt/zoo-bot-server
NODEUSER=root:root
DAEMON=/opt/nodejs/bin/npm
DAEMON_ARGS="start"
...
{
    if [ -f $PIDFILE ]; then
        log_daemon_msg "  --->  $DESC is already running, phantom pidfile $PIDFILE"
        return 1
    fi
    
    start-stop-daemon --start --chdir $CHDIR --chuid $NODEUSER --make-pidfile --pidfile $PIDFILE --background --exec $DAEMON -- \
        $DAEMON_ARGS \
...
#
# Function that kills the daemon/service and the children
#
do_kill()
{
    if [ ! -f $PIDFILE ]; then
    ...
    kill $(pstree -p $(cat $PIDFILE) | grep -oP '(?<=\()[0-9]+(?=\))' | tr "\n" " ")
...
do_status() {
    ...
    if [ -n "$RUNNING" ]; then
        if [ $ISPIDACTIVE ]; then 
            log_success_msg "$INIT_SCRIPT_NAME_NOEXT (--chuid $NODEUSER) is running"
            exit 0
        fi
    else
        if [ -f $PIDFILE ]; then
            log_success_msg "$INIT_SCRIPT_NAME_NOEXT (--chuid $NODEUSER) is running, phantom pidfile $PIDFILE"
            exit 0
        else
            log_success_msg "no instance of $INIT_SCRIPT_NAME_NOEXT (--chuid $NODEUSER) found"
            exit 3
        fi
    fi
}
